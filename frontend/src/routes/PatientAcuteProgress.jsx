...
const PatientAcuteProgress = () => {
    ...
    useEffect(() => {
        const savedData = localStorage.getItem(`patient-acuteprogress-${id}`);
        if (savedData) {
    ...
    if (Object.keys(filteredNoteData).length > 0) {
      localStorage.setItem(`patient-acuteprogress-${id}`, JSON.stringify(filteredNoteData));
    } else {
      localStorage.removeItem(`patient-acuteprogress-${id}`);
    }
    ...
    return (
        <div className="container mt-4 d-flex assessment-page" style={{ cursor: readOnly ? 'not-allowed' : 'text' }}>
            ...
            <div className="ms-4 flex-fill">
                ...
                {/* Treatment/Procedures/Special Instructions */}
                <Card className="assessment-card">
                    <Card.Header className="assessment-card-header">
                        <h4 className="assessment-card-title">Treatment/Procedures/Special Instructions</h4>
                        <button 
                            className="clear-section-btn"
                            onClick={() => {
                                handleAnswerChange('treatment', '');
                                handleAnswerChange('procedures', '');
                                handleAnswerChange('specialInstructions', '');
                            }}
                        >
                            Clear
                        </button>
                        ...
                        <Card.Body className="assessment-card-body">
                            <div className="mb-3">
                                <Form.Group className="mb-3">
                                    <label className="question-label">Treatment:</label>
                                    <Form.Control
                                        as="textarea"
                                        rows={10}
                                        value={answers.treatment || ''}
                                        onChange={(e) => !readOnly && handleAnswerChange('treatment', e.target.value)}
                                        disabled={readOnly}
                                    />
                                </Form.Group>
                            </div>
                            ...
                            <div className="mb-3">
                                <Form.Group className="mb-3">
                                    <label className="question-label">Procedures:</label>
                                    <Form.Control
                                        as="textarea"
                                        rows={10}
                                        value={answers.procedures || ''}
                                        onChange={(e) => !readOnly && handleAnswerChange('procedures', e.target.value)}
                                        disabled={readOnly}
                                    />
                                </Form.Group>
                            </div>
                            ...
                            <div className="mb-3">
                                <Form.Group className="mb-3">
                                    <label className="question-label">Special Instructions:</label>
                                    <Form.Control
                                        as="textarea"
                                        rows={10}
                                        value={answers.specialInstructions || ''}
                                        onChange={(e) => !readOnly && handleAnswerChange('specialInstructions', e.target.value)}
                                        disabled={readOnly}
                                    />
                                </Form.Group>
                            </div>
...
export default PatientAcuteProgress;
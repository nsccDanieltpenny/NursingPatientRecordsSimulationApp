...
export default function ClassCodeEnrollment() {
  ...
  const validateClassCode = async (code) => {
    ...
    if (code.toUpperCase().startsWith("REQ-")) {
      setIsValid(true);
      setValidationMessage(
        "Instructor request code - you will be added to approval queue",
      );
      return;
    }
    ...
    try {
      const response = await api.get(
        `/api/classes/verify/${code.toUpperCase()}`,
      );
      setIsValid(true);
  ...
  return (
    <div style={styles.container}>
      <div style={styles.logoCircle}>
        <img src={logo} alt="CARE app logo" style={styles.logoImage} />
      </div>
      ...
      <form style={styles.form} onSubmit={handleSubmit}>
        ...
        <label htmlFor="classCode" style={styles.formLabel}>
          Class Code
        </label>
        <input
        ...
        {isValidating && (
          <div style={styles.validatingMessage}>Validating...</div>
        )}
        ...
        <div style={styles.instructorSection}>
          ...
          <p style={styles.instructorText}>
            Contact your administrator to receive an instructor request code
          </p>
        </div>
      </form>
    </div>
